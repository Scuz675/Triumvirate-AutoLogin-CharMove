-- === Merged autologin character-save helpers ===
function Autologin_OnCharactersLoad()
	Autologin_Load();
	local selected = Autologin_Table[Autologin_SelectedIdx];
	if (not selected) then
		if AutologinSaveCharacterButton then AutologinSaveCharacterButton:Hide(); end
		return;
	end

	if AutologinSaveCharacterButton then AutologinSaveCharacterButton:Show(); end
	if (selected.character == '-') then return end

	local charIndex = tonumber(selected.character);
	if charIndex and charIndex > 0 and charIndex <= GetNumCharacters() then
...
function Autologin_EnterWorld()
	if (Autologin_SelectedIdx and AutologinSaveCharacterButton and AutologinSaveCharacterButton:GetChecked()) then
		Autologin_Table[Autologin_SelectedIdx].character = CharacterSelect.selectedIndex;
		Autologin_Save();
	end
	EnterWorld();
end
-- === End merged autologin character-save helpers ===
...
function CharacterSelect_OnKeyDown(self,key)
	...
	elseif ( key == "UP" or key == "LEFT" ) then
		...
		if ( numChars > 1 ) then
			local displayIndex = Triumvirate_GetDisplayIndexForReal(self.selectedIndex) or 1;
			if ( displayIndex > 1 ) then
				CharacterSelectUI.id = Triumvirate_GetRealIndexForDisplay(displayIndex - 1);
			else
				CharacterSelectUI.id = Triumvirate_GetRealIndexForDisplay(numChars);
			end
	...
	elseif ( key == "DOWN" or key == "RIGHT" ) then
		...
		if ( numChars > 1 ) then
			local displayIndex = Triumvirate_GetDisplayIndexForReal(self.selectedIndex) or 1;
			if ( displayIndex < numChars ) then
				CharacterSelectUI.id = Triumvirate_GetRealIndexForDisplay(displayIndex + 1);
			else
				CharacterSelectUI.id = Triumvirate_GetRealIndexForDisplay(1);
			end
			GlueFrameFadeOut(CharacterSelect, VX_FADE_REFRESH, CharacterSelectButton_OnClick_Wait);
		end
	end
...
-- === Triumvirate visual character order helpers ===
-- Saves display order inside the saved account-list data as:
-- #CO:Scuz,Altname,Otheralt;
Triumvirate_DisplayToReal = {};
Triumvirate_RealToDisplay = {};

local function Triumvirate_TrimName(value)
...
local function Triumvirate_StripSavedCharacterOrder(raw)
	raw = raw or "";
	raw = string.gsub(raw, "#CO:[^;]*;", "");
	return raw;
end

local function Triumvirate_GetSavedCharacterOrderRaw()
	local raw = GetSavedAccountList() or "";
	local order = string.match(raw, "#CO:([^;]*);") or "";
	return order;
end

local function Triumvirate_SaveSavedCharacterOrderRaw(value)
	local raw = GetSavedAccountList() or "";
	raw = Triumvirate_StripSavedCharacterOrder(raw);
	if value and value ~= "" then
		raw = raw .. "#CO:" .. value .. ";";
	end
	SetSavedAccountList(raw);
end

local function Triumvirate_ReadCharacterOrderNames()
	local raw = Triumvirate_GetSavedCharacterOrderRaw();
	local names = {};
	for name in string.gmatch(raw, "([^,]+)") do
		name = Triumvirate_TrimName(name);
...
function Triumvirate_BuildCharacterOrder()
	local numChars = GetNumCharacters();
	Triumvirate_DisplayToReal = {};
	Triumvirate_RealToDisplay = {};

	local desiredNames = Triumvirate_ReadCharacterOrderNames();
	local byName = {};
	local used = {};

	for realIndex = 1, numChars, 1 do
		local name = GetCharacterInfo(realIndex);
		if name then
			byName[name] = realIndex;
		end
	end

	-- First: saved/preferred order by character name.
	for _, wantedName in ipairs(desiredNames) do
		local realIndex = byName[wantedName];
		if realIndex and not used[realIndex] then
			table.insert(Triumvirate_DisplayToReal, realIndex);
			used[realIndex] = true;
		end
	end

	-- Then: any new/renamed/missing characters in Blizzard's original order.
	for realIndex = 1, numChars, 1 do
		if not used[realIndex] then
			table.insert(Triumvirate_DisplayToReal, realIndex);
			used[realIndex] = true;
		end
	end

	for displayIndex, realIndex in ipairs(Triumvirate_DisplayToReal) do
		Triumvirate_RealToDisplay[realIndex] = displayIndex;
	end
end

function Triumvirate_GetRealIndexForDisplay(displayIndex)
	return Triumvirate_DisplayToReal and Triumvirate_DisplayToReal[displayIndex] or displayIndex;
end

function Triumvirate_GetDisplayIndexForReal(realIndex)
	return Triumvirate_RealToDisplay and Triumvirate_RealToDisplay[realIndex] or realIndex;
end

function Triumvirate_SaveCharacterOrder()
	local numChars = GetNumCharacters();
	local names = {};

	for displayIndex = 1, numChars, 1 do
		local realIndex = Triumvirate_GetRealIndexForDisplay(displayIndex);
		local name = GetCharacterInfo(realIndex);
		if name and name ~= "" then
			table.insert(names, name);
		end
	...
	-- Store visual order in the saved-account-list string, not accountName/password data.
	-- This avoids touching the login/password fields.
	Triumvirate_SaveSavedCharacterOrderRaw(value);
end

function Triumvirate_UpdateCharacterOrderButtons()
	if not TriumvirateCharMoveUpButton or not TriumvirateCharMoveDownButton then return; end

	local numChars = GetNumCharacters();
	local realIndex = CharacterSelect and CharacterSelect.selectedIndex or 0;
	local displayIndex = Triumvirate_GetDisplayIndexForReal(realIndex);

	if (not displayIndex) or numChars <= 1 or realIndex <= 0 then
		TriumvirateCharMoveUpButton:Disable();
		TriumvirateCharMoveDownButton:Disable();
		return;
	end
	...
	if displayIndex <= 1 then
		...
		TriumvirateCharMoveUpButton:Enable();
	end

	if displayIndex >= numChars then
		TriumvirateCharMoveDownButton:Disable();
	else
		TriumvirateCharMoveDownButton:Enable();
	end
end

function Triumvirate_MoveSelectedCharacter(delta)
	local numChars = GetNumCharacters();
	if numChars <= 1 then return; end

	Triumvirate_BuildCharacterOrder();

	local realIndex = CharacterSelect and CharacterSelect.selectedIndex or 0;
	local displayIndex = Triumvirate_GetDisplayIndexForReal(realIndex);
	if not displayIndex then return; end

	local newDisplayIndex = displayIndex + delta;
	if newDisplayIndex < 1 or newDisplayIndex > numChars then return; end

	local oldReal = Triumvirate_DisplayToReal[displayIndex];
	local swapReal = Triumvirate_DisplayToReal[newDisplayIndex];
	Triumvirate_DisplayToReal[displayIndex] = swapReal;
	Triumvirate_DisplayToReal[newDisplayIndex] = oldReal;

	Triumvirate_SaveCharacterOrder();
	UpdateCharacterList();
	CharacterSelect_SelectCharacter(realIndex, 1);
	Triumvirate_UpdateCharacterOrderButtons();

	if DEFAULT_CHAT_FRAME then
		local name = GetCharacterInfo(realIndex) or "Character";
		DEFAULT_CHAT_FRAME:AddMessage("Triumvirate Login: saved character order, moved " .. name .. ".");
	end
end
-- === End Triumvirate visual character order helpers ===
...
function UpdateCharacterSelection(self)
	...
	local realIndex = self.selectedIndex;
	local displayIndex = Triumvirate_GetDisplayIndexForReal(realIndex);
	if ( displayIndex and (displayIndex > 0) and (displayIndex <= MAX_CHARACTERS_DISPLAYED) ) then
		_G[_G["CharSelectCharacterButton"..displayIndex]:GetName().."OnSelect"]:Show();
	end
...
function UpdateCharacterList()
	...
	for displayIndex=1, numChars, 1 do
		local realIndex = Triumvirate_GetRealIndexForDisplay(displayIndex);
		local name, race, class, level, zone, sex, ghost, PCC, PRC, PFC = GetCharacterInfo(realIndex);
		local button = _G["CharSelectCharacterButton"..index];

		-- Button IDs are real character indexes, while button position is our visual order.
		...
		-- Setup paid service buttons. Their IDs also need real character indexes.
		_G["CharSelectCharacterCustomize"..index]:SetID(realIndex);
		_G["CharSelectRaceChange"..index]:SetID(realIndex);
		_G["CharSelectFactionChange"..index]:SetID(realIndex);
	...
	if ( numChars == 0 ) then
		...
		if TriumvirateCharMoveUpButton then TriumvirateCharMoveUpButton:Disable(); end
		if TriumvirateCharMoveDownButton then TriumvirateCharMoveDownButton:Disable(); end
	else
	...
	if ( CharacterSelect.selectLast == 1 ) then
		...
		CharacterSelect_SelectCharacter(Triumvirate_GetRealIndexForDisplay(numChars) or numChars, 1);
		Triumvirate_UpdateCharacterOrderButtons();
		return;
	end

	if ( (CharacterSelect.selectedIndex == 0) or (CharacterSelect.selectedIndex > numChars) ) then
		CharacterSelect.selectedIndex = Triumvirate_GetRealIndexForDisplay(1) or 1;
	end
	CharacterSelect_SelectCharacter(CharacterSelect.selectedIndex, 1);
	Triumvirate_UpdateCharacterOrderButtons();
end
